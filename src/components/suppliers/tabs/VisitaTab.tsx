...
import { Search, Star, MapPin, Phone, Plus, User, ChevronDown, MapPinned, Link as LinkIcon } from "lucide-react"
        ...
        <div className="flex h-full bg-transparent p-6 gap-6">
            ...
            <div className="w-[300px] bg-white rounded-2xl p-6 space-y-8 overflow-y-auto shadow-sm border-r-2 border-blue-600/20">
                <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between py-2">
                            <Label htmlFor="favorites-visit" className="text-sm font-medium text-gray-700 cursor-pointer">
                                Exibir apenas favoritos
                            </Label>
                            <Checkbox id="favorites-visit" className="w-5 h-5 rounded border-gray-300 data-[state=checked]:bg-blue-600 data-[state=checked]:border-blue-600 shadow-none" />
                        </div>
                    </div>
                </div>
                ...
                <Collapsible defaultOpen className="space-y-4">
                    ...
                    <CollapsibleContent className="space-y-6 pt-2">
                        <div className="space-y-4">
                            <Label className="text-sm font-semibold text-gray-900 block">Tipo de visita</Label>
                            <div className="space-y-3">
                                {['Eventos Agro', 'Experiências locais', 'Visitas técnicas'].map((type) => (
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar">
                    {visits.map((visit) => (
                        <div key={visit.id} className="bg-white border border-gray-100 rounded-2xl p-6 hover:shadow-xl hover:shadow-gray-200/50 transition-all group relative">
                            <div className="flex items-center gap-6">
                                {/* Left Section: Info */}
                                <div className="flex items-center gap-4 flex-1">
                                    <button className="text-gray-300 hover:text-orange-400 transition-colors shrink-0">
                                        <Star className={cn("h-6 w-6", visit.isFavorite && "fill-orange-400 text-orange-400")} />
                                    </button>
                                    <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center shrink-0">
                                        <MapPinned className="h-6 w-6 text-blue-600" />
                                    </div>
                                    <div className="space-y-1">
                                        <div className="text-[10px] font-medium text-gray-400 uppercase tracking-wider">Visita</div>
                                        <div className="flex items-center gap-2">
                                            <h3 className="font-bold text-gray-900 text-xl">{visit.name}</h3>
                                            <span className="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-bold uppercase">{visit.type}</span>
                                        </div>
                                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm">
                                            <a href={visit.location_link} className="flex items-center gap-1.5 text-blue-600 font-medium hover:underline">
